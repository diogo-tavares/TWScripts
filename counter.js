(function(window) {
    ...
    function fnCalculateTroopCount() {
        ...
        var outputSummary = { 
            ...
            'Half Atack': { group: 'Offensive', criteria: [{ unit: 'offense', minpop: 10000, maxpop: 14999 }], descID: 5 }, 
            'Quarter Atack': { group: 'Offensive', criteria: [{ unit: 'offense', minpop: 5000, maxpop: 9999 }], descID: 6 }, 
        ...
        for (var sItem in outputSummary) {
            if (outputSummary.hasOwnProperty(sItem) && summary[sItem].coords.length) {
                villageDataJson[sItem] = summary[sItem].coords;
            }
        }

        docSource += '<script type="text/javascript">';
        docSource += 'var coordsData = ' + JSON.stringify(villageDataJson) + ';';
        ...
        docSource += 'var container = document.getElementById("coords_container");';
        docSource += 'var html = "";';
        docSource += 'if(coordsData[id]) {';
        docSource += '  coordsData[id].forEach(function(v) {';
        docSource += '    html += \'<a href="\' + v.url + \'">\' + v.name + \'</a><br>\';';
        docSource += '  });';
        docSource += '} else { html = "<i>Nenhuma aldeia encontrada.</i>"; }';
        docSource += 'container.innerHTML = html;';
        docSource += '};';
        docSource += '</script>';
        ...
        docSource += '<tr><td style="box-sizing:border-box;width:100%; padding: 5px;">';
        docSource += '<div id="coords_container" style="width:100%; box-sizing:border-box; height:120px; overflow-y:auto; background-color:#fff5da; border:1px solid #7d510f; padding:5px; font-size:11px;">Clique num grupo de exércitos (Ex: Full Atack) para ver a lista de aldeias.</div>';
        docSource += '</td></tr>';
        docSource += '</table>';
